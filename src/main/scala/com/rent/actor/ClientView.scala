...
object ClientView {
    ...
    case class PostMessage(message: Message, answerTo: ActorRef[Event]) extends Event
    ...
    private def init(ctx: ActorContext[Event], controller: ChatService): Behavior[Event] = {
        Behaviors.receiveMessage {
            case NewClient(clientPort, clientNickName) =>
                ...
                clientInCluster = Customer(clientPort, clientNickName, ctx.self)
    ...
    private def running(ctx: ActorContext[Event], clients: IndexedSeq[ActorRef[ClientView.Event]], controller: ChatService,
        ...
        Behaviors.receiveMessage {
            case ClientsUpdated(newClients) =>
                ...
                newClients.foreach(actor => if (actor != ctx.self) actor ! MyInfo(Customer(clientPort, clientNickName, ctx.self), ctx.self))
            ...
            case PostMessage(message, answerTo) =>
