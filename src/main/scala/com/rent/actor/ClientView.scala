...
object ClientView {
    ...
    case class PostMessage(message: Message, answerTo: ActorRef[Event]) extends Event with CborSerializable
    ...
    private def running(ctx: ActorContext[Event], clients: IndexedSeq[ActorRef[ClientView.Event]], controller: ChatService): Behavior[Event] =
        Behaviors.receiveMessage {
            case ClientsUpdated(newClients) =>
                ...
                if (check && newClients.size > 0) {
                    Platform.runLater(() => controller.sayClusterThatImAlive(ctx.self))
                }
                check = true
